...
group size each person belongs to, return the groups there are and the people's
    ...
    public List<List<Integer>> groupThePeople(int[] groupSizes) {
        ...
        for (int i = 0; i < groupSizes.length; i++) {
            ...
            if (groupSize == 1) {
                ...
                List<Integer> existingGroup = groups.computeIfAbsent(groupSize, k -> new ArrayList<>());
                existingGroup.add(i);
                if (existingGroup.size() == groupSize) {
                    results.add(existingGroup);
